    ...
    while(boucle){
        ...
        switch(evenement.type){//Type d'évènement
            ...
            case SDL_MOUSEMOTION:
                //Rectangle 1
                if(evenement.motion.x<(positionRect1.x+Reclongueur)
                   && evenement.motion.x>positionRect1.x
                   && evenement.motion.y<(positionRect1.y+Rechauteur)
                   && evenement.motion.y>positionRect1.y){
                    SDL_FillRect(rectangle1, NULL, SDL_MapRGB(fenetre->format, 255, 206, 112));
                }
                //Rectangle 2
                if(evenement.motion.x<(positionRect2.x+Reclongueur)
                   && evenement.motion.x>positionRect2.x
                   && evenement.motion.y<(positionRect2.y+Rechauteur)
                   && evenement.motion.y>positionRect2.y){
                    SDL_FillRect(rectangle2, NULL, SDL_MapRGB(fenetre->format, 255, 206, 112));
                }
                //Rectangle 3
                if(evenement.motion.x<(positionRect3.x+Reclongueur)
                   && evenement.motion.x>positionRect3.x
                   && evenement.motion.y<(positionRect3.y+Rechauteur)
                   && evenement.motion.y>positionRect3.y){
                    SDL_FillRect(rectangle3, NULL, SDL_MapRGB(fenetre->format, 255, 206, 112));
                }
                //Rectangle 4
                if(evenement.motion.x<(positionRect4.x+Reclongueur)
                   && evenement.motion.x>positionRect4.x
                   && evenement.motion.y<(positionRect4.y+Rechauteur)
                   && evenement.motion.y>positionRect4.y){
